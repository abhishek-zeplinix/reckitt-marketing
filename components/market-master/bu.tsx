...
const BUControls = () => {
    ...
    const [selectedBUId, setSelectedBUId] = useState<any>();
    const [isEditMode, setIsEditMode] = useState<boolean>(false);
    const { layoutState } = useContext(LayoutContext);
    const { setAlert, setLoading, isLoading } = useAppContext();
    const { error: buError, validate: validateBU, resetError } = useZodValidation(reviewTypeSchema);
    ...
    const handleSubmit = async () => {
        if (!validateBU(bu)) return;
        setLoading(true);

        if (isEditMode) {
            try {
                const payload = { buName: bu };
                const response = await PutCall(`/mrkt/api/mrkt/marketingBU/${selectedBUId}`, payload);

                if (response.code.toLowerCase() === 'success') {
                    setAlert('success', 'BU successfully updated!');
                    resetInput();
                    fetchData();
                }
            } catch (err) {
            ...
            try {
                const payload = { buName: bu };
                const response = await PostCall('/mrkt/api/mrkt/marketingBU', payload);

                if (response.code.toLowerCase() === 'success') {
                    setAlert('success', 'BU successfully added!');
                    resetInput();
                    fetchData();
                }
            } catch (err) {
    ...
    const onDelete = async () => {
        ...
        try {
            const response = await DeleteCall(`/mrkt/api/mrkt/marketingBU/${selectedBUId}`);

            if (response.code.toLowerCase() === 'success') {
                setRolesList((prevRoles: any) => prevRoles.filter((bu: any) => bu.buName !== selectedBUId));
                fetchData();
                closeDeleteDialog();
                setAlert('success', 'BU successfully deleted!');
            } else {
                setAlert('error', 'Something went wrong!');
                closeDeleteDialog();
            }
        ...
        <>
            <div className="flex flex-column justify-center items-center gap-2">
                ...
                {buError ? (
                    ...
                    <i>Enter a BU you want to add.</i>
                </small>}
                <SubmitResetButtons onSubmit={handleSubmit} label={isEditMode ? 'Update BU' : 'Add BU'} loading={isLoading}/>
            </div>
            ...
            >
                ...
                <div className="flex flex-column w-full surface-border p-2 text-center gap-4">
                    ...
                    <div className="flex flex-column align-items-center gap-1">
                        <span>Are you sure you want to delete this bu? </span>
