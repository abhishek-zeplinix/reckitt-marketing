...
const DownloadBarcodeButton = ({ url = '', type = '', ids = [], onSuccess = () => {}, onError = (message: any) => {} }: Options) => {
    ...
    return <Button type="button" icon={`pi ${isDownloading ? 'pi-spin pi-spinner' : 'pi-download'}`} label="Barcodes" onClick={generateBarcodes} data-pr-tooltip="Generate barcodes" disabled={isDownloading} />;
};
