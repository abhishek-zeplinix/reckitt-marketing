    ...
    return (
        <div key={`file_${file.assetId}_${file.name}`} className="col-4 sm:col-3 lg:col-2 file-item">
            <div className="flex flex-column p-2 shadow-2 border-round align-items-center text-center border-round-sm" onClick={() => onFileCheck(file)}>
                <Checkbox key={`file_check_${file.assetId}_${file.name}`} className="file-checkbox" onClick={() => onFileCheck(file)} checked={!!find(selectedFiles, { assetId: file.assetId })} />
                <i className="pi pi-eye file-view primary-color cursor-pointer" onClick={() => viewImage(file)} />
            </div>
...
const MyFileUpload = ({ isVisible, onSelect }: any) => {
    ...
    const dialogFooter = (
        <div className="dialog-footer p-2 flex justify-content-between">
            <div>{selectedFiles.length > 0 && <Button className="btn-small" label="Clear selection" text onClick={() => setSelectedFiles([])} />}</div>
            <div>
    ...
    const emptyTemplate = (
        <div className="drop-field flex align-items-center flex-column my-3">
            <div className="flex align-items-center">
                <Button
                    className="btn-small custom-choose-btn mr-2"
                    severity="secondary"
                    text
                    raised
                    onClick={() => {
                ...
                <Button className="btn-small mr-2" severity="secondary" text>
                    Add from URL
                </Button>
            </div>
            <span style={{ color: 'var(--text-color-secondary)' }} className="my-2">
                    ...
                    <ScrollPanel className="flex align-items-center" style={{ width: '100%', height: '20rem', marginTop: '1.5rem' }}>
                        ...
                        <div className="grid p-2">
                                ...
                                <div key={`file_${file.assetId}_${file.name}`} className="col-4 sm:col-3 lg:col-2 file-item">
                                    <div className="flex flex-column p-2 shadow-2 border-round align-items-center text-center border-round-sm" onClick={() => onFileCheck(file)}>
                                        <Checkbox
                                            ...
                                            onChange={() => onFileCheck(file)}
                                            checked={find(selectedFiles, { assetId: file.assetId }) ? true : false}
                                            style={{ visibility: file.status == 'uploaded' ? 'visible' : 'hidden' }}
                                        ></Checkbox>
                                        {/* <MemoizedFileItem key={`file_image_${file.assetId}_${file.name}`} file={file} /> */}
                                    ...
                                    <div className="file-info flex flex-column align-items-center mb-2">
                                        <p className="m-0 mt-2 sub-desc text-elipsis">{file.name}</p>
                                        <p className="m-0 sub-desc">
                                            {get(file, 'type', '').toUpperCase()} • {formatBytes(file.size)}
                                        </p>
                                    </div>
                            ...
                            {fileList.map((file: Asset) => (
                                <MemoizedItemTemplate key={`file_${file.assetId}_${file.name}`} file={file} onFileCheck={onFileCheck} selectedFiles={selectedFiles} viewImage={viewImage} />
                            ))}

                            {fileList.length == 0 && !isFileLoading && (
                                <div className="flex align-items-center justify-content-center w-full h-3rem">
                                    <p>No media found</p>
                                </div>
                            )}
                        </div>
            ...
            >
                <div className="grid grid-nogutter" style={{ height: '100%' }}>
                    <div className="col-12 md:col-6 lg:col-4" style={{ backgroundColor: '#1f1f1f', color: '#fff', padding: '1rem', boxSizing: 'border-box' }}>
                        ...
                        <Divider />
                        <p>
                            <strong>Name:</strong> {assetFile?.name}
                        </p>
                        <p>
                            <strong>Details:</strong> {get(assetFile, 'type', '').toUpperCase()} • {assetFile?.width} x {assetFile?.height} • {formatBytes(assetFile?.sizeInBytes)}
                            <br />
                            Added on {moment(assetFile?.createdAt).format('lll')}
                        </p>
                        <p>
                            <strong>Used in:</strong> {assetFile?.usedIn || 'No referenced found'}
                        </p>
                    </div>
                </div>
            </Dialog>
